...
class SKMultilineLabel: SKNode {
    ...
    var shouldShowBox:Bool = false {didSet {update()}}
    //display objects
    var rect:SKShapeNode?
    var labels:[SKLabelNode] = []
    
    init(text:String, labelWidth:Int, pos:CGPoint, fontName:String="ChalkboardSE-Regular",fontSize:CGFloat=10,fontColor:SKColor=SKColor.black,leading:Int=10, alignment:SKLabelHorizontalAlignmentMode = .center, shouldShowBox:Bool = false)
    {
        self.text = text
        ...
        self.shouldShowBox = shouldShowBox
    ...
    func showBox() {
        if (!shouldShowBox) {return}
        if let rect = self.rect {
            self.removeChildren(in: [rect])
        }
        self.rect = SKShapeNode(rect: CGRect(x: -CGFloat(5 + labelWidth/2), y: -CGFloat(5 + labelHeight/2), width: CGFloat(labelWidth + 10), height: CGFloat(labelHeight + 10)))
        if let rect = self.rect {
            rect.strokeColor = SKColor.darkGray
            rect.fillColor = SKColor.darkGray
            rect.alpha = 0.5
